...
import { Mail, Phone, GraduationCap, BookOpen, Award, Microscope, FlaskConical, Users, ExternalLink, MapPin, Calendar, Building } from "lucide-react"
...
export default async function DepartmentDetailPage({
  ...
  const initialTab = ["about", "laboratories", "equipment", "services", "staff"].includes(searchParamsResolved?.tab || "")
    ? (searchParamsResolved!.tab as "about" | "laboratories" | "equipment" | "services" | "staff")
    : "about"
            ...
            <Tabs defaultValue={initialTab} className="w-full">
              <TabsList className="flex w-full mb-8 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg p-1 shadow-sm overflow-x-auto gap-1">
                ...
                >
                  ...
                  <span className="text-sm font-medium whitespace-nowrap">Laboratories</span>
                </TabsTrigger>
                <TabsTrigger 
                  value="equipment" 
                  className="flex items-center gap-2 data-[state=active]:bg-slate-100 data-[state=active]:dark:bg-slate-700 data-[state=active]:text-slate-900 data-[state=active]:dark:text-slate-100 py-2 px-3 rounded-md transition-colors text-slate-600 dark:text-slate-400 flex-shrink-0"
                        ...
                        <AnimatedSection key={service.id} animation="fade-up" delay={index * 0.1}>
                          <Card className="border border-slate-200 dark:border-slate-700 shadow-sm bg-white dark:bg-slate-800 hover:shadow-md transition-all duration-300">
                            <CardHeader>
                              <div className="flex items-start justify-between">
                                <div className="flex-1">
                                  <CardTitle className="text-lg text-slate-900 dark:text-slate-100">{service.name}</CardTitle>
                                  <CardDescription className="mt-2 text-slate-600 dark:text-slate-400">{service.description}</CardDescription>
                                </div>
                                {service.price && (
                                  <Badge variant="outline" className="ml-4 border-slate-300 dark:border-slate-600 text-slate-700 dark:text-slate-300">
                                    ${service.price}
                                  </Badge>
                                )}
                              </div>
                            ...
                            <CardContent>
                              <div className="flex items-center justify-between">
                                {service.duration && (
                                  <span className="text-sm text-slate-500 dark:text-slate-400">Duration: {service.duration}</span>
                                )}
                                <Link href="/contact">
                                  <Button size="sm" className="bg-slate-900 hover:bg-slate-800 dark:bg-slate-100 dark:hover:bg-slate-200 text-white dark:text-slate-900">
                                    Request Service
                                  </Button>
                                </Link>
                              </div>
              ...
              <TabsContent value="staff" className="mt-0">
                <div className="space-y-6">
                  ...
                  {department.staff && department.staff.length > 0 ? (
                    <div className="grid md:grid-cols-2 gap-6">
                      {department.staff.map((member: any, index: number) => (
                        <AnimatedSection key={member.id} animation="fade-up" delay={index * 0.1}>
                          <Card className="border border-slate-200 dark:border-slate-700 shadow-sm bg-white dark:bg-slate-800 hover:shadow-md transition-all duration-300 group">
                            <CardHeader>
                              <div className="flex items-start gap-4">
                                <div className="relative h-16 w-16 flex-shrink-0">
                                  <Image
                                    ...
                                    fill
                                    className="object-cover rounded-full ring-2 ring-slate-200 dark:ring-slate-600"
                                  />
                                </div>
                                <div className="flex-1 min-w-0">
                            ...
                            <CardContent className="space-y-3">
                              ...
                              <div className="flex flex-col gap-2">
                                {member.email && (
                                  <a
                                    ...
                                    className="text-sm text-slate-700 dark:text-slate-300 hover:text-slate-900 dark:hover:text-slate-100 transition-colors flex items-center gap-2 p-2 bg-slate-50 dark:bg-slate-700/50 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-700"
                                  >
                                    <Mail className="h-4 w-4" />
                                    {member.email}
                                  </a>
                                ...
                                {member.phone && (
                                  <div className="text-sm text-slate-700 dark:text-slate-300 flex items-center gap-2 p-2 bg-slate-50 dark:bg-slate-700/50 rounded-lg">
                                    <Phone className="h-4 w-4" />
                                    {member.phone}
                                  </div>
                                )}
                              ...
                              <div className="pt-2 border-t border-slate-200 dark:border-slate-700">
                                <Link 
                                  ...
                                  className="w-full"
                                >
                                  <Button size="sm" variant="outline" className="cursor-pointer w-full group-hover:bg-slate-900 group-hover:text-white dark:group-hover:bg-slate-100 dark:group-hover:text-slate-900 transition-colors">
                                    View Profile
                                  </Button>
                                </Link>
                              </div>
